...
public class ImportFromYGOPRO {
	...
	public static void insertGameplayCardFromYGOPRO(JSONObject current) throws SQLException {
		...
		SQLiteConnectionEnum.getImplementation().replaceIntoGamePlayCard(wikiID, name, type, passcode, desc, attribute, race, linkval, level,
	...
	public static void insertCardSetsForOneCard(JSONArray sets, String name, int wikiID)
		...
		for (int i = 0; i < sets.length(); i++) {
			...
			SQLiteConnectionEnum.getImplementation().replaceIntoCardSet(set_code, set_rarity, set_name, wikiID, set_price, name);

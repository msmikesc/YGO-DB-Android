...
public class Util {
	...
	public static void checkSetCounts() throws SQLException {
		ArrayList<SetMetaData> list = SQLiteConnection.getObj().getAllSetMetaDataFromSetData();

		for (SetMetaData setData : list) {
			int countCardsinList = SQLiteConnection.getObj().getCountDistinctCardsInSet(setData.set_name);
		...
		ArrayList<String> setNames = SQLiteConnection.getObj().getDistinctSetNames();
		...
		for (String setName : setNames) {
			...
			int cardsInSet = SQLiteConnection.getObj().getCountDistinctCardsInSet(setName);
	...
	public static CardSet findRarity(String priceBought, String dateBought, String folderName, String condition,
		...
		if (setRarities.size() == 0) {
			...
			// check for name
			setIdentified.id = SQLiteConnection.getObj().getCardIdFromTitle(cardName);
	...
	public static void checkForIssuesWithSet(String setName) throws SQLException {
		...
		ArrayList<String> cardsInSetList = SQLiteConnection.getObj().getSortedCardsInSetByName(setName);

...
import com.example.ygodb.backend.analyze.AnalyzeCompareToDragonShieldCSV;
...
public class CsvConnection {
	...
	public static OwnedCard getOwnedCardFromExportedCSV(CSVRecord current) throws SQLException {
		...
		if(wikiID == null) {
			wikiID = SQLiteConnection.getObj().getCardIdFromTitle(name);
		}
	...
	public static OwnedCard getOwnedCardFromTCGPlayerCSV(CSVRecord current) throws SQLException {
		...
		CardSet setIdentified = SQLiteConnection.getObj().getFirstCardSetForCardInSet(name, setName);
		...
		ArrayList<SetMetaData> metaData = SQLiteConnection.getObj().getSetMetaDataFromSetName(setName);
	...
	public static void insertGamePlayCardFromCSV(CSVRecord current, String defaultSetName) throws SQLException {
		...
		SQLiteConnection.getObj().replaceIntoGamePlayCard(passcode, name, type, passcode, lore, attribute, race, linkValue,
	...
	public static void insertCardSetFromCSV(CSVRecord current, String defaultSetName) throws SQLException {
		...
		int wikiID = SQLiteConnection.getObj().getCardIdFromTitle(name);
		// try skill card
		if (wikiID == -1) {
			wikiID = SQLiteConnection.getObj().getCardIdFromTitle(name + " (Skill Card)");
			if (wikiID != -1) {
		...
		SQLiteConnection.getObj().replaceIntoCardSet(cardNumber, rarity, setName, wikiID, price, name);
	}
